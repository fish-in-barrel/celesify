...
def run() -> None:
    ...
    log(SERVICE, f"Missing-value strategy: drop rows -> removed {rows_removed} rows ({removal_pct:.4f}%).")
    ...
    log(SERVICE, f"Train split shape: {train_df.shape}; class counts: {train_counts}")
    log(SERVICE, f"Test split shape: {test_df.shape}; class counts: {test_counts}")
